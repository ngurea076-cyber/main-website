...
class DatabaseSeeder extends Seeder {
    ...
    private function importNeon(string $path): bool {
        ...
        foreach($data['settings']??[] as $row) DB::table('settings')->updateOrInsert(['key'=>$row['key']],['value'=>is_string($row['value'])?$row['value']:json_encode($row['value']),'created_at'=>now(),'updated_at'=>$row['updated_at']??now()]);
