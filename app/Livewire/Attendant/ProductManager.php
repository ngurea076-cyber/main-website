...
class ProductManager extends Component {
    ...
    public string $search=''; public ?int $editingId=null; public string $title=''; public string $brand='';
    public string $description=''; public string $price=''; public string $old_price=''; public string $stock_status='in_stock';
    public ?int $category_id=null; public bool $featured=false; public array $newImages=[];
    ...
    private function uniqueSlug(string $title, ?int $except=null): string { $base=Str::slug($title); $slug=$base; $i=2; while(Product::where('slug',$slug)->when($except,fn($q)=>$q->whereKeyNot($except))->exists()) $slug=$base.'-'.$i++; return $slug; }
